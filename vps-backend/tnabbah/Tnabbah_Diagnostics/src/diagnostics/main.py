...
# Add CORS middleware
# - If ALLOWED_ORIGINS is unset, fall back to "*" (open dev mode).
# - Per CORS spec, credentials cannot be combined with the "*" wildcard,
#   so disable credentials whenever the origin list is wide-open.
_cors_origins = ALLOWED_ORIGINS if ALLOWED_ORIGINS else ["*"]
_cors_allow_credentials = not ALLOW_ALL_ORIGINS and bool(ALLOWED_ORIGINS)
...
logger.info(
    ...
    _cors_origins, _cors_allow_credentials,
)
...
async def scan_vehicle(request: ScanRequest) -> AIReport:
    ...
    assert data_loader is not None  # narrows Optional for type checker
    ...
    try:
        ...
        # Capture KB into a local so the closure does not depend on the
        # global Optional[DataLoader], which Pylance can't narrow.
        _dtc_kb_local: Dict[str, Any] = data_loader.dtc_kb

        def _category_display(dtc_key: str) -> str:
            raw_cat = _dtc_kb_local.get(dtc_key, {}).get("category")
...
@app.post("/api/scan-from-mqtt")
async def scan_from_mqtt(
    request: ScanFromMqttRequest,
    ...
    *not* paused; we just freeze one self-consistent moment.
    """
    ensure_owner(uid, request.user_id)
    listener = get_mqtt_listener()
    if listener is None:

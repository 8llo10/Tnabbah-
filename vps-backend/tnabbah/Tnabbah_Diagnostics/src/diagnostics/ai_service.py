...
from .formatters.severity import _limit_words
...
_DTC_CODE_RE = re.compile(r"\bP[0-9A-F]{4}\b", re.IGNORECASE)
# Cut overview after the code if the model starts explaining the fault (belongs on DTC card).
_OVERVIEW_DTC_EXPLAIN_RE = re.compile(
    r"^(?P<head>.*?\b(?:كود\s+)?P[0-9A-F]{4}\b)\s+"
    r"(?:الذي|التي|ويشير|يُشير|مما|دلالة|بسبب|يعني|يشير\s+إلى).+",
    re.IGNORECASE | re.DOTALL,
)


def _first_sentence_ar(text: str) -> str:
    """Keep a single Arabic overview sentence (first sentence terminator)."""
    text = (text or "").strip()
    if not text:
        return ""
    for sep in (".", "؟", "!", "?"):
...
def _sanitize_overview_ar(text: str) -> str:
    """Holistic one-liner: readings + DTC code mention only, no fault explanation."""
    text = _first_sentence_ar(text)
    if not text:
        return ""
    m = _OVERVIEW_DTC_EXPLAIN_RE.match(text)
    if m:
        return m.group("head").strip()
    return text


def build_holistic_overview_ar(
    ...
    *,
    ...
    """
    Deterministic holistic summary for the smart-summary card when AI is off
    or did not return overview_ar.
    """
    abnormal: List[str] = []
    for r in readings or []:
        status = str(r.get("status") or "NORMAL").upper()
        is_anom = bool(r.get("is_anomaly")) or status not in (
            "NORMAL",
            "OK",
            "SUCCESS",
        )
        if not is_anom:
            continue
        name = str(r.get("name_ar") or r.get("pid_code") or "").strip()
        if name and name not in abnormal:
    ...
    if not abnormal and not codes:
        return "جميع القراءات ضمن النطاق الطبيعي."

    parts: List[str] = []
    if not abnormal:
        parts.append("جميع القراءات طبيعية")
    elif len(abnormal) == 1:
        parts.append(f"جميع القراءات طبيعية باستثناء {abnormal[0]}")
    elif len(abnormal) == 2:
        parts.append(f"جميع القراءات طبيعية باستثناء {abnormal[0]} و{abnormal[1]}")
    else:
        parts.append(f"معظم القراءات طبيعية مع {len(abnormal)} تنبيهات من المستشعرات")

    if len(codes) == 1:
        parts.append(f"مع وجود كود {codes[0]}")
    elif len(codes) > 1:
        listed = " و".join(codes[:3])
        if len(codes) > 3:
            listed += f" و{len(codes) - 3} أخرى"
        parts.append(f"مع وجود أكواد {listed}")

    return _limit_words(" ".join(parts), max_words)
...
class AIService:
    ...
    def explain_pids(
        ...
        if not client:
            overview = build_holistic_overview_ar(readings, dtcs)
            return {"interpretations": [], "overview_ar": overview} if overview else None
        ...
        prompt_lines.append(
            ...
            '{"overview_ar":"جملة واحدة فقط تلخّص حالة القراءات بشكل عام (≤ 25 كلمة). '
            'إن وُجدت أكواد أعطال اذكر رقم الكود فقط (مثل: \\"مع وجود كود P0104\\") '
            'دون أي شرح لمعنى الكود أو سببه — الشرح يُكتب لاحقاً في بطاقة الكود نفسه. '
            'لا تكتب أكثر من جملة واحدة، ولا تستعمل علامة نقطة في المنتصف.", '
        ...
        try:
            ...
            try:
                ...
                if overview:
                    # Defensive: one sentence, DTC code only — no fault meaning
                    # (per-DTC smart_insight_ar owns the explanation).
                    overview = _sanitize_overview_ar(overview)
                if not overview:
                    overview = build_holistic_overview_ar(readings, dtcs)
                if overview:

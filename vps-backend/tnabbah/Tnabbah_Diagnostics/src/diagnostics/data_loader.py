...
class DataLoader:
    ...
    def __init__(self, base_path: Optional[str] = None):
        ...
        if base_path is None:
            ...
            if env_data_path:
                resolved_path = Path(env_data_path)
            else:
                # src/diagnostics/data_loader.py -> project_root/data
                resolved_path = Path(__file__).resolve().parents[2] / "data"
        else:
            resolved_path = Path(base_path)

        self.base_path = resolved_path.resolve()
        # Knowledge bases default to empty dicts (not None) so type checkers
        # and consumers can rely on Dict[str, Any] semantics before load.
        self.dtc_kb: Dict[str, Any] = {}
        self.pid_kb: Dict[str, Any] = {}
    ...
    def update_learning_state(self, pids: Dict[str, float], severity_score: float):
        ...
        self.learning_state.system_health_history.append({
            ...
            "health_score": float(100 - severity_score),
        })

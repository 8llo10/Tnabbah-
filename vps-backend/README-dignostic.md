...
| `SUPABASE_JWT_SECRET` | سرّ HS256 من Supabase Auth (مطلوب لحماية endpoints المستخدم) | فارغ |
| `SUPABASE_JWT_AUDIENCE` | جمهور التوكن المتوقع | `authenticated` |
| `TNABBAH_DISABLE_AUTH` | `true` لتعطيل JWT في التطوير المحلي فقط | `false` |
| `TNABBAH_DEV_USER_ID` | معرّف المستخدم الافتراضي عند تعطيل المصادقة | فارغ |

> **حماية**: كل endpoints القراءة/الكتابة المرتبطة بمستخدم
> (`/api/save-report`, `/api/delete-report`, `/api/reject-report`,
> `/api/reports/saved/{user_id}`, `/api/reports/pending/{user_id}`,
> `/api/reports/all/{user_id}`, `/api/scan-from-mqtt`,
> `/api/mqtt/snapshot`) تتطلب رأس
> `Authorization: Bearer <Supabase access token>`؛ الـ `sub` المستخرج
> يجب أن يطابق `user_id` في الطلب وإلا تُرجع `403`.

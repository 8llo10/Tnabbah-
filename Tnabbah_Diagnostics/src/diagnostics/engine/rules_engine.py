...
class RulesEngine:
    ...
    def analyze_pids(self, pids: Dict[str, float], dtcs: Optional[List[str]] = None, learning_state: Optional[Dict] = None, vehicle_info: Optional[Dict] = None) -> PIDAnalysisResult:
        """
        ...
        # 🔧 Calculate DTC impact on health
        # CRITICAL: -30%, HIGH: -20%, MEDIUM: -10% (each DTC)
        ...
        if dtcs:
            for dtc_code in dtcs:
                ...
                if dtc_code_upper in self.dtc_kb:
                    ...
                    logger.debug(f"   DTC {dtc_code_upper} ({severity}): -{severity=='CRITICAL' and 30 or severity=='HIGH' and 20 or 10}%")

        # Cap the DTC penalty so many low-severity codes can't drive overall_health
        # far negative before the final 0..100 clamp (which would erase signal
        # about how bad the underlying system scores actually are).
        dtc_health_penalty = min(dtc_health_penalty, 60.0)

        # 🔧 FIXED: Calculate overall_health using balanced formula
        # Health = 100 - (average*0.5 + worst*0.5) - dtc_penalty
        # This gives weight to all systems but prioritizes the worst one
        if system_scores:
            system_values = list(system_scores.values())
            avg_system_score = sum(system_values) / len(system_values)
            worst_system_score = max(system_values)
            # Balanced formula: 50% average, 50% worst
            overall_health = 100 - (avg_system_score * 0.5 + worst_system_score * 0.5) - dtc_health_penalty
        elif anomalies:
            # Fallback: if no system scores but have anomalies, use worst anomaly severity
            worst_anomaly_severity = max(a.severity_score for a in anomalies)
            overall_health = max(0, 100 - worst_anomaly_severity - dtc_health_penalty)
        else:
            # No issues found, but DTCs exist
    ...
    def calculate_overall_severity(
        ...
        Any live sensor (``0x..`` PID) with analyzer_status ``CRITICAL`` forces at least CRITICAL overall,
        so unmapped PIDs cannot under-report vs. KB critical bands.
        """

        if anomalies:
            for a in anomalies:
                ast = str(getattr(a, "analyzer_status", None) or "").upper()
                pid = str(getattr(a, "pid_code", "") or "")
                if ast == "CRITICAL" and _pid_code_is_live_sensor(pid):
                    return SeverityLevel.CRITICAL

        if not system_scores and not dtcs:
            return SeverityLevel.LOW

        # Check for critical DTCs first
        critical_dtcs = ["P0217", "P0401", "P0503", "P0300", "P0128", "P0171", "P0172", "P0420"]
        high_dtcs = ["P0171", "P0172", "P0128", "P0300"]

        if any(dtc in critical_dtcs for dtc in dtcs):
            # Check if multiple critical issues
            critical_count = sum(1 for dtc in dtcs if dtc in critical_dtcs)
            if critical_count >= 2:
                return SeverityLevel.CRITICAL
            return SeverityLevel.HIGH

        # Calculate from system scores
        max_score = max(system_scores.values()) if system_scores else 0

        # If we have HIGH DTCs, at least HIGH severity
        if any(dtc in high_dtcs for dtc in dtcs):
            return _max_severity(SeverityLevel.HIGH, self._score_to_severity(max_score))

...
export default function HomeScreen() {
    ...
    const [carId, setCarId] = useState<string | null>(activeCarId);
    ...
    const activeCarIdRef = useRef<string | null>(activeCarId);
    ...
    useEffect(() => {
        const update = async () => {
            ...
            if (!activeCarId && cachedCarId) {
                setCarId(cachedCarId);
            }
        ...
        const setupMqttListener = async () => {
            try {
                ...
                const onMessage = (topic: string, message: Buffer) => {
                    ...
                    if (currentActiveCarId && incomingCarId !== currentActiveCarId) {
                        return;
                    }

                    if (incomingCarId && incomingCarId === currentActiveCarId) {
                        setCarId(incomingCarId);
                    }
    ...
    const performDirectScan = async () => {
        try {
            ...
            const scannedCarId =
                activeCarId || carId || vehicleScannerService.getCachedCarId();
